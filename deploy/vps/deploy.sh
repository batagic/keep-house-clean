...
#
#   ./deploy/vps/deploy.sh           # Chỉ API lên VPS
#   ./deploy/vps/deploy.sh --all     # git push + API lên VPS
#   ./deploy/vps/deploy.sh --help
#
# Lần đầu (một lần): ./deploy/vps/setup-ssh.sh
#
...
Deploy Kho Thóc — một lệnh từ Mac
  ...
  ./deploy/vps/deploy.sh           Deploy API lên VPS (rsync + docker rebuild)
  ./deploy/vps/deploy.sh --all     Thêm git push origin main (GitHub Pages)
...
ssh_cmd "env SKIP_GIT_PULL=1 INSTALL_ROOT='${REMOTE_ROOT}' VERIFY_PATTERN='${VERIFY_PATTERN:-}' bash -s" \
  < "${REPO_ROOT}/deploy/vps/deploy-api.sh"
