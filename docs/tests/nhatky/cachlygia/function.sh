...
TEST_FILE="${SCRIPT_DIR}/function.test.mjs"

if node --test "$TEST_FILE"; then
  pass "NK-ISO-F suite — node --test function.test.mjs"
else
  fail "NK-ISO-F suite — node --test function.test.mjs"
fi
...
assert_file_contains \
  ...
  "${REPO_ROOT}/kho-thoc-api/migrations/004_family_id.sql" \
  'family_id'
