...
|------------|------|----------|
| `nhat-ky.html`, `admin/`, `assets/` | GitHub Pages | `git push` |
| `kho-thoc-api/src/`, migrations | VPS Docker | `git pull` + `rsync` + **`docker compose up -d --build`** |
...
| **B** Chỉ API | `kho-thoc-api/src/` | **Có** | — |
| **C** API + DB | `migrations/` | **Có** + migrate | Tùy |
