...
- Thêm màn hình / flow nhập **passcode**; lưu token session vào `localStorage`.
- Mọi `fetch` kèm header `Authorization` (hoặc tương đương) sau khi đăng nhập.
...
**Multi-tenant:** mọi bảng có `family_id`; API luôn filter theo gia đình từ session/JWT sau khi đăng nhập passcode.
