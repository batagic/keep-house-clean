...
Sau khi VPS đã chạy → xem [operations.md](./operations.md) (deploy tính năng mới, xử lý sự cố).
...
Export **Profiles** và **Logs** từ Google Sheets → đặt vào `kho-thoc-api/data/`:
...
| [kho-thoc-api/README.md](../kho-thoc-api/README.md) | API contract |
| `kho-thoc-api/scripts/setup-db.sh` | Tạo user + DB |
| `deploy/vps/nginx/apinhatkyvumua.taho.cat.conf` | Config Nginx HTTPS |
